# englisheducation — CLAUDE.md (Claude Code)

이 레포는 임용고시 영어교육론 OX 퀴즈 + 개념정리 정적 웹앱이다. Claude Code는 이 `CLAUDE.md`를 1차 진입 문서로 사용한다. Codex는 `AGENTS.md`를 사용하며, 두 파일은 도구 framing만 다르고 섹션·순서·규칙은 동일하다. 새 규칙은 양쪽 모두 갱신한다.
