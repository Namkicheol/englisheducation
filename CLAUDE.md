...
| `.kw2` | 부키워드 | `color:#00796b; font-weight:700; font-family:'JetBrains Mono',monospace; font-size:.88em; border-bottom:2px solid rgba(0,121,107,.45)` |
...
### 효과음 (sounds.js)
- 정답 클릭 시 → `playOk()` (밝은 두 음)
- 오답 클릭 시 → `playNg()` (낮은 두 음)
- 결과 화면 시 → `playResult(점수)` (점수별 멜로디)
- iOS Safari 포함 모바일 자동 잠금 해제 포함

### 결과 팝업 (score-popup.js)
- 퀴즈 완료 시 `showScorePopup(s, TOTAL)` 호출
- 점수별 이모지·타이틀·컨페티 자동 적용
- localStorage에 최고 기록 저장 + 신기록 배지 표시

### 스크립트 삽입 (</body> 바로 앞, 필수)
```html
<script src="./score-popup.js"></script>
<script src="./sounds.js"></script>
```

### showScorePopup 호출 위치 (render 함수 내 cur>=TOTAL 처리)
```js
...
- **sounds.js + score-popup.js 반드시 포함** (모든 OX 퀴즈 파일)
