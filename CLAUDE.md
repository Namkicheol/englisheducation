...
## 🤖 신규 페이지 작성 시 백그라운드 에이전트 실행 (필수)

`_study.html` 또는 OX 파일을 **신규 작성하거나 대폭 수정**할 때 아래 에이전트를 백그라운드로 실행한다.

### 트리거 조건

| 트리거 | 실행 에이전트 |
|--------|-------------|
| 새 `_study.html` 챕터 작성·수정 | **기출이력 추가 에이전트** |
| 새 OX 파일(`*_ox.html`) 작성·문항 추가 | **OX 기출각도 보강 에이전트** |

### 에이전트 작업 명세

**기출이력 추가 에이전트**가 해야 할 일:
...
6. 변경 파일만 git add → commit → push

**OX 기출각도 보강 에이전트**가 해야 할 일:
