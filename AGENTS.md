...
- 작업 완료 전 최소한 아래를 검색한다(rg/Grep 모두 사용 가능).
...
- 썸네일은 `blog/thumbnails/`에 저장한다. 로컬 도형 합성으로 급조하지 말고, 사용자가 요구한 경우 image model(Canva / Pencil / Gemini 등 도구 무관) 결과물을 기준으로 만든다.
...
- 브라우저(Claude in Chrome / Codex in Chrome)로 직접 업로드한다. 사용자가 크롬에 티스토리 로그인을 유지한다.
- 절차: ① `.md` 파일 작성 → ② 브라우저(Playwright / Browser 도구)로 티스토리 에디터의 **마크다운 모드**에 붙여넣기 → ③ 발행 → ④ 발행 후 화면에서 사용자가 직접 사진(Gemini 생성 이미지)을 첨부한다.
- 에디터 모드 전환 금지. 기본/마크다운/HTML 간 전환 시 하이라이트 스타일이 전부 소멸한다. 소멸 시 모드 전환 없이 마크다운 원본을 다시 붙여넣는다.

## 전역 지침 참조

전역 지침(`~/.claude/CLAUDE.md` / `~/.codex/AGENTS.md`)의 한국어 용어 정책과 코딩 행동 원칙이 이 레포에도 적용된다.
